...
class Lphelper
{
	...
	public static function Menu($position,$ul='',$li='',$a='',$icon_position='top',$lang=false){
		...
		if ($lang==false) {
			 $menus=cache()->remember($position,60,function(){
			 	return $menus=Menu::where('position',Lphelper::$position)->where('status',1)->where('lang','en')->first();
			 });
			
		}
		else{
			 $menus=cache()->remember($position.Session::get('locale'),60,function(){
			 	return $menus=Menu::where('position',Lphelper::$position)->where('status',1)->where('lang',Session::get('locale'))->first();
			 });	
